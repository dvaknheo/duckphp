...
| 四 高级话题 | 会集成 | 改框架行为、调优、排错、维护文档、自己实现用户/管理员体系、看清 `Ext\*` 那一族 | 13（含 4-12/4-13「实现用户/管理员系统」与 [4-14「`Ext\*` 扩展类」](guide/ext-classes.md)；`4-11` 是已知缺号，见 §7） |
...
- 全量 `php vendor/bin/phpunit --no-coverage` → **`OK (95 tests, 862 assertions)`**（约 6 分钟）；
- 覆盖率 **`4726/4726 (100.00%)`**：`XDEBUG_MODE=coverage php vendor/bin/phpunit`（跑全量、顺便写 `test_coveragedumps/`）→ 再 `XDEBUG_MODE=coverage php vendor/bin/phpunit tests/support.php` 生成 `test_reports/index.html`；判「有没有漏测」用 `php docs/scripts/covagg.php`（按源文件合并全部 dump，输出 `TOTAL … lines x/y` 与有缺口的文件）。⚠️ **中途 Fatal 的测试不会写自己的 dump**，覆盖率会假降（实测见过 `4774/4898 (97.47%)`）——先确认全量没有红，再信覆盖率；⚠️ 删过类/改过测试文件后，`test_coveragedumps/` 里的旧 dump 会让 covagg 的总数虚高，**先 `rm -rf test_coveragedumps` 再跑全量**；
...
- **形式**：卷-章号，单数字形式已全部废弃。范围：第一卷 `1-1`–`1-7`；第二卷 `2-1`–`2-20`；第三卷 `3-1`–`3-7`；第四卷 `4-1`–`4-10` 与 `4-12`–`4-14`；附录 `A`–`D`（术语表 / 片段 / 迁移 / FAQ）。
- **⚠️ 当前缺号：`4-11` 空着**（原来那章「过时与冷门的扩展类」已并入 `4-14`「`Ext\*` 扩展类」，为的是**先不重排章号**）。下次集中重排时把 `4-12`/`4-13`/`4-14` 顺次前移成 `4-11`/`4-12`/`4-13`，并同步总目录里的 `//TODO` 标记、本节的表和全库 `第 4-1x 章` 引用。**在那之前别把 4-11 当成「待写的章」**。
...
- **`Ext\*` 的内容只在 4-14 讲**（作者裁定）：卷二/卷三只留「怎么用」的 1~3 行 + 指针，类名、`ext` 装配、选项与坑集中到 `ext-classes.md`；`Ext\RouteHookManager` 也归 4-14（卷二用 `Route::_()->addRouteHook()` / `dumpAllRouteHooksAsString()`）。往卷二加 `Ext\` 内容前先看这条。
...
- **改完的判据**：① 扫 `docs/zh/guide/*.md` 里所有「链接文字带 `第 X-Y 章`」的引用，用「文件名 → 章号」表反查 → **0 处不一致**；② 总目录每行的章号与目标文件的 H1 对得上；③ `python3 docs/scripts/check-doc-links.py docs/zh` 仍 0 死链；④ 各章 H1 无重号（当前允许 `4-11` 这一个已知缺号，见上）。
