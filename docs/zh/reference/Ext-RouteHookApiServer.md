...
`RouteHookApiServer` 是“API 服务器”路由扩展：把形如 `Namespace/Service.method`（`/` 路径 + `.` 动作）的请求映射到 `api_server_namespace` 下的服务类并调用，结果以 JSON 输出；并预设跨域头与 JSON 错误响应。挂在 `prepend-inner`（默认路由前接管 API 类请求）。
...
\DuckPhp\Ext\RouteHookApiServer::_()->init([
    ...
    'api_server_base_class' => '\\MyProject\\Api\\Base',
], $app);
...
未命中处理：开启 `api_server_404_as_exception` 时抛 `ReflectionException("404")`，否则返回 `false`。
