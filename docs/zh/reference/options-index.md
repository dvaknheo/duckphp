...
**跳转**：[A](#a) · [C](#c) · [D](#d) · [E](#e) · [F](#f) · [G](#g) · [H](#h) · [I](#i) · [J](#j) · [L](#l) · [M](#m) · [N](#n) · [O](#o) · [P](#p) · [R](#r) · [S](#s) · [U](#u) · [V](#v) · [W](#w)
...
| 选项 | 默认值 | 来源类 | 说明 |
|---|---|---|---|
| `globaladmin_enable_callback_singleton` | `true` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 回调是 `[类名, 方法]` 时，是否先把类名换成 `类名::_()` 单例实例。 |
| `globaladmin_ext_view_data_callback` | `null` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 追加视图数据的回调（键缺失时**不报错**，跳过即可；与上面三个「必需」回调不同）。 |
| `globaladmin_is_authed_redirect` | `true` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | `login()`/`logout()` 完成后是否自动 302（登录跳 `urlForHome()`、退出跳 `urlForLogin()`）。 |
| `globaladmin_local_service` | `null` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 返回 `AdminServiceInterface` 实现的回调（键缺失时 `localService()` 抛异常）。 |
| `globaladmin_login_service` | `null` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 返回 `AdminLoginServiceInterface` 实现的回调。 |
| `globaladmin_login_session` | `null` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 返回 `AdminSessionInterface` 实现的回调。 |
| `globaladmin_need_login_callback` | `null` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 未登录时的自定义处理；配了就**不会**走默认的 302/JSON，只回调再 `exit()`。 |
| `globaladmin_url_home` | `null` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 后台首页 URL；未配时取 App 的同名上下文选项 `url_admin_home`，再退回 `'/'`。 |
| `globaladmin_url_login` | `null` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 后台登录 URL；未配时退回 `'/'`（这是 `throwLoginOn()` 的 302 目标）。 |
| `globaladmin_url_logout` | `null` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 后台退出 URL；未配时取 App 的上下文选项 `url_admin_logout`，再退回 `'/'`。 |
| `globaladmin_view_file_footer` | `null` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 后台页脚视图文件（非空才渲染，结果并入 `__view_data.footer`）。 |
| `globaladmin_view_file_header` | `null` | [DuckPhp\GlobalAdmin\GlobalAdmin](GlobalAdmin-GlobalAdmin.md) | 后台页眉视图文件（非空才渲染，结果并入 `__view_data.header`）。 |
| `globaluser_enable_callback_singleton` | `true` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 回调是 `[类名, 方法]` 时，是否先把类名换成 `类名::_()` 单例实例。 |
| `globaluser_ext_view_data_callback` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 追加视图数据的回调（键缺失时**不报错**，跳过即可；与上面三个「必需」回调不同）。 |
| `globaluser_is_authed_redirect` | `true` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | `register()`/`login()`/`logout()` 完成后是否自动 302（注册/登录跳 `urlForHome()`、退出跳 `urlForLogin()`）。 |
| `globaluser_local_service` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 返回 `UserServiceInterface` 实现的回调（键缺失时 `localService()` 抛异常）。 |
| `globaluser_login_service` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 返回 `UserLoginServiceInterface` 实现的回调。 |
| `globaluser_login_session` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 返回 `UserSessionInterface` 实现的回调。 |
| `globaluser_need_login_callback` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 未登录时的自定义处理；配了就**不会**走默认的 302/JSON，只回调再 `exit()`。 |
| `globaluser_url_home` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 站内首页 URL；未配时取 App 的上下文选项 `url_user_home`，再退回 `'/'`。 |
| `globaluser_url_login` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 登录页 URL；未配时退回 `'/'`（这是 `throwLoginOn()` 的 302 目标）。 |
| `globaluser_url_logout` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 退出 URL；未配时取 App 的上下文选项 `url_user_logout`，再退回 `'/'`。 |
| `globaluser_url_register` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 注册页 URL；未配时退回 `'/'`。 |
| `globaluser_view_file_footer` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 用户页脚视图文件（非空才渲染，结果并入 `__view_data.footer`）。 |
| `globaluser_view_file_header` | `null` | [DuckPhp\GlobalUser\GlobalUser](GlobalUser-GlobalUser.md) | 用户页眉视图文件（非空才渲染，结果并入 `__view_data.header`）。 |
