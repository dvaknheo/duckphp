...
- `default`：共享实例所在的默认容器名（框架里通常是 `#public`）；
- `publics`：被标记为“可跨 Phase 共享”的类清单（见下）。

当需要取某类的实例（`Class::_()`）时，`_GetObject()` 的查找顺序是：**当前 Phase 里的实例 → 若该类在 `publics` 中，则到 default 容器找公共实例 → 都找不到则自动 `new` 并登记到目标容器**。这构成了“根应用与子应用各自独立、又有少量可共享组件”的多实例格局。

DuckPHP 的根 Phase 名常为 `''`（空串），公共/共享容器默认名在 `KernelTrait` 记为 `#public`。子应用的 Phase 名形如 `父Phase:命名空间/名称`。

## 类信息

- 命名空间：`DuckPhp\Core`
- 声明：`class PhaseContainer`
- 关键属性：`public static $instance`；实例属性 `$containers`、`$current`、`$default`、`$publics`。
...
// 挂/查公共组件
$c->addPublicClasses([MyShared::class => true]);

// 取对象（不存在则创建到当前位置；是公共类则在 default 中找或建）
...
- 默认容器（`#public`）中的实例是全局共享；要被共享的类必须通过 `addPublicClasses($classes)` 声明，否则 `_GetObject()` 不会回退去 default 容器找。
...
输出当前容器全部状态（current/default/publics/containers）到 stdout，便于调试。

### 公共实例方法

    public function _GetObject(string $class, ?object $object = null): object
核心查找+创建：先在 current 容器找；该 Class 在 publics 时再去 default 找；没有再（在目标容器）new/登记实例。

    public function setDefaultContainer($class)
设置默认（共享）容器名 —— 即放公共实例的地方。

    public function addPublicClasses($classes)
把若干类标记为 public（键为类名、值为真即可）。
例 `addPublicClasses([Some::class => true])`。

    public function removePublicClasses($classes)
从 public 名单移除这些类（值为数组列表）。
...
- [DuckPhp\Core\KernelTrait](Core-KernelTrait.md) — 设置根 Phase(`''`)与公共容器名(`#public`)，并调用 initContainer/addPublicClasses 装配
