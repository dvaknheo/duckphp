...
> 前置：[第 29 章 重写与覆盖](overriding.md)（文件/类覆盖）、[第 32 章 容器与相位内部机制](container-phases.md)。预计 20 分钟。
> 示例：`Ext\CallableView`/`Ext\JsonView`（换 View 实现）、`Ext\RedisCache`（换 Cache 实现）、`AppWithAllOptions.php`（选项全表）。
...
| **文件级**   | `getOverrideableFile()` 的相位回退（`view/`、`config/`、`res/`）                  | 按相位逐层      | 子应用覆盖视图/配置/资源（[第 29 章](overriding.md)）   |     |
| **单例级**   | `Xxx::_($newObject)`                                                     | 当前相位       | 换 View 引擎、换 Cache 实现、测试替身                |     |
| **系统调用级** | `Helper::system_wrapper_replace()`（`SystemWrapper`）                      | 全进程        | 拦 `header`/`exit`/`session_start`，做测试或常驻 |     |
| **组件级**   | 组件自己的选项：`database_class`、`view_*`、`*_skip_replace`                       | 该组件        | 换 Db 实现、关掉自动替换                           |     |
...
框架的动作是：把 `override_class` 的值当成新类，`$class::_(new $class)->init($options, $context)`（`KernelTrait::init()` 开头），并把 `override_from` 记成原类——所以覆盖类能拿到"我是从谁被换过来的"。

控制器级更常用的是 `controller_class_map`（[第 9 章](routing.md)）：它是**类名 → 类名**的映射，在路由解析出类名之后、实例化之前生效。`Helper::replaceController()` 就是往这个映射里写一条。

### 3. 单例级：`Xxx::_($new)`

所有 `ComponentBase` 子类都通过 `SingletonExTrait::_($object = null)` 走容器：不传参取实例，**传对象就替换实例**。这就是扩展换实现的统一姿势：
...
意义有两个：**测试**（输出/跳转可断言，[第 23 章](testing.md)）与**移植**（换掉某个平台不一致的行为）。框架内部所有输出都走它（`Helper::header()`、`Helper::setcookie()`、`Html`/`Json` 输出），所以替换一处、全局生效。
...
| 换整份 Session 行为 | `session_prefix` + `SessionTrait` 子类 | 覆盖 `SuperGlobal` |
| 关掉某个扩展 | `ext` 里置 `EXT_DISABLE`（`0`） | 卸载代码 |
| 每次请求重建组件 | `ext` 里置 `EXT_RENEW`（`3`） | 手动 `reInit()` |

`EXT_*` 的五个取值见[第 33 章](custom-component.md)；`DbManager` 的 `database_class`、`App` 的 `override_class` 都属于这一类。
...
var_dump(App::_()->options['controller_class_map']);  // 映射真的写进去了吗（选项白名单！第 5 章）
App::_()->getOverrideableFile('view', 'main.php');    // 文件级：实际命中的是哪个文件
```

最常见的两个原因：**取实例时用的不是同一个相位**（[第 32 章](container-phases.md)），以及**选项根本没活下来**（组件用它自己的 `$options` 白名单过滤，[第 33 章](custom-component.md)）。
...
| 现象                               | 原因                                  | 改法                                                                   |
| -------------------------------- | ----------------------------------- | -------------------------------------------------------------------- |
| 替换后完全没反应                         | 调用处用了 `new`，没走容器                    | 一律 `Xxx::_()`（[第 29 章](overriding.md)）                               |
| 子应用里替换了，父应用没变                    | 单例是**按相位**存的                        | 替换在目标相位里做，或改用共享组件（[第 28 章](component-sharing.md)）                    |
| `system_wrapper_replace()` 拦不住输出 | 代码直接调了原生 `header()`/`echo`+`exit`   | 走 `Helper::header()` / `Helper::exit()`                              |
| 选项写了但没生效                         | 不在该组件的 `$options` 白名单里              | 看[第 33 章](custom-component.md)的白名单机制；或用 `$hidden_options` 的读法        |
| `controller_class_map` 不生效       | 键写的是短名                              | 键必须是**类的全限定名**（`MyProj\Controller\UserController`）                   |
| `override_class` 导致配置看起来变了       | 换的是整个应用类                            | 注意新类的 `$options` 与 `override_from`；选项仍会合并（[第 5 章](configuration.md)） |
| 覆盖视图没生效                          | 文件名/相位名不匹配                          | `getOverrideableFile()` 打印实际命中文件（[第 29 章](overriding.md)）            |
| 换了 `Cache` 实现但取不到缓存              | `RedisCache` 需要 `RedisManager` 也初始化 | 两个组件一起声明（[第 20 章](cache.md)）                                         |

## 下一步

- [第 33 章 开发组件与扩展](custom-component.md)：写出自己的可替换实现。
- [第 36 章 常驻进程与内嵌 HTTP](http-server.md)：系统包装在长跑进程里的意义。
- [第 40 章 性能调优与排错手册](troubleshooting.md)：替换没生效时按"症状 → 排查路径"走一遍。
