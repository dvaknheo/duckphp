...
> 前置：[第 8 章 四层架构与调用规范](layers.md)、[第 9 章 路由进阶](routing.md)。预计 20 分钟。
...
`Base` 是工程自己的控制器基类（`tests/data_for_tests/ZAllDemo/src/Controller/Base.php`），只有 `use SingletonTrait;` 一行——**为了 `MainController::_()` 能当单例用**。
...
| 基类 | 位置 | 用途 |
|---|---|---|
| `DuckPhp\Foundation\Controller\Base` | `src/Foundation/Controller/Base.php` | 最简：只 `use SingletonTrait` |
| `DuckPhp\Foundation\Controller\ActionBase` | 同上目录 | Action 的基类 |
| `DuckPhp\Foundation\Controller\UserControllerBase` | 同上目录 | 前台用户控制器（[第 16 章](external-auth.md)） |
| `DuckPhp\Foundation\Controller\AdminControllerBase` | 同上目录 | 后台管理员控制器（[第 16 章](external-auth.md)） |
...
Helper::Show404();                // 直接以 404 结束（框架默认 404 机制见第 18 章）
```
...
页眉页脚（布局）用 `Helper::setViewHeadFoot('header', 'footer')` 指定，通常写在控制器的 `__construct()` 里——`demo/public/demo.php` 的 `MainController::__construct()` 就是这么做的（渲染顺序是 头 → 视图 → 尾，详见[第 11 章](views.md)）。
...
控制器里只留输入输出：`$user = UserAction::_()->login($name, $password);`（Action 的完整分工见[第 16 章](external-auth.md)）。
...
```php
// 路由：'/note/{id:\d+}' => '...\NoteController@show'（第 9 章）
public function show()
{
...
- [第 11 章 视图与模板](views.md)：视图文件怎么定位、页眉页脚、转义。
- [第 15 章 表单与数据验证](validator.md)：输入校验的三种口径。
- [第 16 章 会话与用户/管理员体系](external-auth.md)：`UserControllerBase` / `AdminControllerBase` 与 Action。
