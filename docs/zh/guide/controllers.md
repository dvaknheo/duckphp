...
> 前置：[第 2-1 章 四层架构与调用规范](layers.md)、[第 2-2 章 路由进阶](routing.md)。预计 20 分钟。
...
2. **调业务**：把参数整理好交给 Business（后面几章都在讲这条线：[第 2-5 章 数据库](database.md)、[第 2-6 章 模型层](model.md)、[第 2-7 章 Helper](helper.md)）；
...
| **路由参数**        | `Helper::Parameter('id')`                                                                   | 路由映射里 `{id}`、`*` 通配、正则捕获组匹配出来的参数（[第 2-2 章](routing.md)） |
...
Helper::Show404();                // 直接以 404 结束（框架默认 404 机制见第 2-11 章）
```
...
页眉页脚（布局）用 `Helper::setViewHeadFoot('header', 'footer')` 指定，通常写在控制器的 `__construct()` 里——`demo/public/demo.php` 的 `MainController::__construct()` 就是这么做的（渲染顺序是 头 → 视图 → 尾，详见[第 2-4 章](views.md)）。
...
```php
// 路由：'/note/{id:\d+}' => '...\NoteController@show'（第 2-2 章）
public function show()
{
...
| 在 `echo` 之后又 `Helper::header()` 报「已发送输出」   | 输出已经开始                                      | 先发头再输出；或用 [`Runtime`](../reference/Core-Runtime.md) 的输出缓冲（[第 2-10 章](lifecycle.md)） |
