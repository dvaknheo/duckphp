...
| 登录后仍显示未登录 | [`GlobalUser`](../reference/GlobalUser-GlobalUser.md) 的回调配置 | `globaluser_login_session` 等键名要对（旧名 `user_callback_for_session` / `user_callback_get_*` 都已失效）（[第 2-18 章](user.md)） |
