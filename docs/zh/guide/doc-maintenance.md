...
约定：**没写的页面不挂链接**——在总目录里用「纯文本 + `⏳`」，写完再换成链接，这样链接校验永远为零死链。**当前指南 47 章 + 4 附录、参考手册 114 篇全部落稿**，所以 `⏳` 只在**新增**章节/页面还没写完时才会出现（全书 grep 应只剩这条约定本身的文字）。
...
先用 `check-doc-links.py` 定位：目标还没写完就改成**纯文本 + `⏳`**、等写完再换回链接；本该存在却写错了路径，就直接改对（约定见本节 §4）。
...
| 链接校验报坏链 | 指向了还没写的章/页 | 未落稿的写成「纯文本 + `⏳`」、写完再换链接；写错路径的直接改对（§4） |
