...
[`DuckPhp\Component\RedisCache`](../reference/Component-RedisCache.md)（`src/Component/RedisCache.php`）在 Redis 上实现了同一组方法：
...
[`DuckPhp\Component\RedisManager`](../reference/Component-RedisManager.md)（`src/Component/RedisManager.php`）管理一组 Redis 连接：
...
[`DuckPhp::initComponentsOfInner()`](../reference/DuckPhp.md) 检测到 `local_redis` 后会 `createLocalObject(RedisManager::class)` 并重新 `init()`（`src/DuckPhp.php` 第 146–149 行），于是子应用相位里的 `RedisManager::_()` 是独立实例。

### 什么时候**别**用框架缓存

- **请求内的临时数据**：用 [`Runtime`](../reference/Core-Runtime.md)（随请求清空，无一致性负担）。
