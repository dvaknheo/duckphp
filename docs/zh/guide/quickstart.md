...
> 前置：[第 1-3 章 目录结构与四层架构](project-structure.md)。预计 25 分钟。
> 示例写法在仓库里都有同类实现：`skeleton/src/`（脚手架生成的四层骨架，本章照着它写）与 `demo/`（可运行的多入口示例应用，`tests/ZAllDemoTest.php` 会起内置服务器跑它）。
...
| `http://127.0.0.1:8080/`               | 走 `MainController::index()`（欢迎页控制器，第 2-3 章） |
