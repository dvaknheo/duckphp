...
> **框架自带一层兜底**：`Route` 的选项 `controller_fix_mistake_path_info`（默认 `true`，源码 `src/Core/Route.php` 第 387–404 行 `getPathInfo()`）会在 `PATH_INFO` 为空**且** `SCRIPT_NAME` 恰好等于 `/index.php` 时，用 `parse_url(REQUEST_URI, PHP_URL_PATH)` 把路径补回 `PATH_INFO`（并写回 `$_SERVER`/SuperGlobal）。
>
