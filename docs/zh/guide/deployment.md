...
> 前置：[第 2 章](install.md)、[第 5 章](configuration.md)。预计 15 分钟。
...
> ⚠️ **但它只对「不像文件」的路径生效**：像 `/res/main.css` 这种带后缀的 URL，内置服务器**不会**交给 `index.php`，而是直接 404 —— 也就是说**由框架代发的静态资源**（[第 28 章](static-resources.md)）在这种启动方式下访问不到。放进 `public/` 的真实文件当然照常由服务器直出。
...
```php
'path_info_compact_enable' => true,     // 由 RouteHookPathInfoCompat 接管（第 9 章）
```
...
日志写不进去时框架**不会报错**（`Logger::log()` 静默返回 `false`），所以这条一定要单独检查（第 6 章）。
...
class App extends DuckPhp
{
    public $options = [
        ...
        'installed' => true,                 // ★ 安装完成后置真（第 31 章）
    ];
}
```

数据库/Redis 口令放设置文件或 `.env`，别写进代码库（第 5 章）。
...
- [ ] 静态资源：放进 `public/` 由服务器直出，或确认 rewrite 后框架代发正常（第 28 章）
- [ ] 日志轮转（`log_file_template` 按天/小时 + 外部 logrotate）
- [ ] HTTPS 与 HSTS；Cookie 的 secure/httponly 按需（第 17 章）
...
| 框架代发的资源 404（nginx 下不该出现） | rewrite 没生效 / 资源前缀不匹配 | 第 28 章的斜杠规则；`res/` 内容可用 `cloneResource()` 部署到 docroot |
| 页面 500 但看不到原因 | `is_debug=false` 且没配 `error_500` | 先看 `runtime/` 日志；临时开 `is_debug` 复现 |
| 日志没生成 | `runtime/` 不可写 | 给 Web 用户写权限（见上） |
| 部署后跳安装页 | `installed` 仍是 `false` | 置 `true`（第 31 章） |

## 下一步

- 第一卷到此结束。接着看[第二卷 · 单一应用](../guide/index.md)：[第 8 章 请求生命周期与钩子点](lifecycle.md) 起。
- 相关：[第 31 章 安装器与 Web 安装流程](installer.md)、第 38 章「多入口·多域名·多 SAPI」（⏳ 撰写中）
