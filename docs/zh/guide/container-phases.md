...
> 前置：[第 25 章 应用树与相位基础](advanced-phase.md)、[第 28 章 组件共享与应用间通信](component-sharing.md)。预计 20 分钟。
...
同一个 `Logger::_()`、同一个 `Lang::_()`，为什么一个到处相同、一个随相位而变？答案全在 `DuckPhp\Core\PhaseContainer` 这一个类里（[参考手册 Core-PhaseContainer](../reference/Core-PhaseContainer.md)）。
...
「相位」就是 `$containers` 的一个键。根相位是空串 `''`；子应用是 `:shop` 这类名字（第 25 章）。`#public` 也是一个桶名——**共享实例并不放在某个相位里，而是放在这个专门的公共桶里**。
...
所以一条规则覆盖全部情况：**非 public 类按当前相位各存一份；public 类全进程只有一份，存在 `#public` 桶里**。`Lang` 属于前者，`Logger`/`Console`/`DbManager` 属于后者——共享与否不取决于类写在哪，而取决于它有没有被 `addPublicClasses()` 标记（见下）。
...
- `DuckPhp::initComponentsOfRoot()` 在此基础上又并入 `DbManager`/`RedisManager`（`EXT_DEFAULT`）与 `GlobalAdmin`/`GlobalUser`/`GlobalEvent`（默认 `EXT_DISABLE`，打开后也是 public）。
- 每个应用自己的 `Route` 走 `initComponentsOfInner()`，**不标 public**——所以每个相位有自己的路由表。
- `options['ext']` 里的扩展走 `initComponentsOfExt()`，同样不标 public——默认各相位一份。

### 可变单例：`::_()` 与 `::_($new)`

`ComponentBase` 使用 `SingletonExTrait`，其 `::_()` 只有一行（`src/Core/SingletonExTrait.php` 16-19 行）：
...
`Foundation\SingletonTrait` 只是 `SingletonExTrait` 的别名（`src/Foundation/SingletonTrait.php` 全文就是 `use SingletonExTrait`），两者完全等价；工程四层基类用的是前者，框架组件用的是后者，语义一模一样。

### 相位名规则与切换 API

| 规则 | 说明 |
|---|---|
| 根相位 | 空串 `''`（`KernelTrait::$ROOT_PHASE`，`src/Core/KernelTrait.php` 69 行） |
...
| 同名相位冲突 | 子应用相位名已被占用时抛 `DuckPhpSystemException`，提示改 `name` 选项（251-256 行） |

切换入口（都在 `KernelTrait`）：`App::Phase($new)` 切桶并返回旧相位（165-175 行）；`App::Root(true)` 取根实例并把相位切回根（104-111 行）；`toThisChild($class)` 按 `options['app'][类]['__phase__']` 切到子相位并返回该子应用（203-212 行）；`SwitchRootPhase()` 用于嵌套场景里重设「谁是根」（131-139 行）。

### 测试辅助：`RestAllContainerForTesting()`

`PhaseContainer::RestAllContainerForTesting()`（`src/Core/PhaseContainer.php` 33-36 行）把整个容器换成一个全新的——测试套件每个用例开头调用它，保证单例不跨用例泄漏。写自己的多应用测试时照抄这个模式即可（第 23 章）。
...
// 4) 手动换某个组件的单例（替换框架行为的底层手段，第 34 章展开）
...
`dumpAllObject()` 的输出格式（`src/Core/PhaseContainer.php` 124-154 行）：先打印 `current`/`default`，再列出 publics 表，然后**逐桶**列出每个实例——public 的类名前带 `*`，类名与实际对象类不一致时括号标出真实类（例如 `DuckPhp\Core\View (DuckPhp\Ext\CallableView)`，说明 View 单例被替换过了）。`tests/data_for_tests/ZAllDemoTest-10360.txt` 就是一份真实 dump，可直接对照。
...
| dump 里某实例的类名带括号 `(OtherClass)` | 该单例被 `::_(新对象)` 替换过 | 正常（如 JsonView 替换 View）；排查替换来源（第 34 章） |
...
- [第 33 章 开发组件与扩展](custom-component.md)：组件怎么声明选项、初始化、挂进路由——本章的容器规则就是它们的运行环境。
- [第 34 章 替换框架行为](replace-behavior.md)：`::_(新实例)`、`system_wrapper_replace()` 等替换手段的系统化。
