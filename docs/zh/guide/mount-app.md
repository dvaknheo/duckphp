...
> 前置：[第 3-1 章 应用树与相位基础](advanced-phase.md)。预计 15 分钟。
...
**不要为了接进来而改它的文件**：需要调整就在 `app` 选项里注入（第 2 步的表），需要改行为就走覆盖（第 3-5 章）。这样它下次升级你仍然能直接替换目录。
...
GET /legacy-shop   → 主应用写的重写规则指到了它（第 3-5 章）
```
...
- [第 3-3 章 静态资源与文档根](static-resources.md)
- [第 3-5 章 重写与覆盖](overriding.md)：不改它的文件，换掉它的视图/配置/控制器
