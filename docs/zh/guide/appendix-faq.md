...
| 问题 | 答案 | 去哪看 |
|---|---|---|
| 想在请求前后做事，用哪个？ | 拦截用路由钩子（pre 链返回真值）或中间件短路（`return` 响应）；前后置装饰用中间件 | [第 2-4 章 路由钩子](../guide/route-hooks.md) |
| 中间件里 `return` 了响应怎么没生效？ | 返回 `null`/`false`（或没写返回值）会被当成「没处理」放行；短路要 `return '内容'` 或 `true` | [第 2-4 章 路由钩子](../guide/route-hooks.md) |
