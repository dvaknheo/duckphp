...
| `Helper` 和全局函数怎么选？ | 视图里用全局函数（`__h`/`__url`/`__l`），PHP 代码里用对应层的 `Helper::` | [第 2-7 章](../guide/helper.md) |
| `::_()` 是什么？ | 可变单例入口：`Xxx::_()` 取实例、`Xxx::_($new)` 替换实例 | [第 4-1 章](../guide/container-phases.md) |
| 为什么某层 Helper 没有我要的方法？ | 那是边界提示：该事不该在这一层做 | [第 2-7 章](../guide/helper.md) |

### 路由与视图

| 问题 | 答案 | 去哪看 |
|---|---|---|
| URL 怎么映射到控制器？ | 最后一段是方法名，前面是类路径，类名自动补 `Controller` 后缀；单段路径落到欢迎类的方法 | [第 2-2 章](../guide/routing.md) |
| 怎么把 URL 绑到指定类@方法？ | `route_map_important`（优先）或 `route_map`（兜底），回调写 `Class@method` | [第 2-2 章](../guide/routing.md) |
| 旧链接怎么兼容？ | `assignRewrite('/old', 'new/path')`——**键必须带前导 `/`** | [第 2-2 章](../guide/routing.md) |
| 视图文件放哪、怎么被找到？ | `view/<视图名>.php`，查找按相位逐层回退（可被父应用覆盖） | [第 2-4 章](../guide/views.md) |
| 怎么加统一的页眉页脚？ | 控制器构造函数里 `Helper::setViewHeadFoot('header', 'footer')` | [第 2-4 章](../guide/views.md) |
| 输出只有「渲染视图」一种吗？ | 四种：视图、`Render()` 取字符串、`ShowJson()`、直接 `echo`；另有 `Show302`/`Show404` | [第 2-3 章](../guide/controllers.md) |
| 怎么防 XSS？ | 所有输出用 `__h()`（翻译+转义用 `__hl()`） | [第 2-4 章](../guide/views.md) |

### 数据

| 问题 | 答案 | 去哪看 |
|---|---|---|
| 数据库怎么配？ | `database`（单条）或 `database_list`（多条/读写分离）；密码走 settings | [第 2-5 章](../guide/database.md) |
| 表前缀怎么处理？ | SQL 里写 `` `'TABLE'` `` 宏，执行前替换成「前缀+表名」 | [第 2-5 章](../guide/database.md) |
| 为什么 `ModelTrait` 的 `add()` 不能直接调？ | 它是 `protected`（刻意）：对外暴露什么由你自己开 public 方法 | [第 2-6 章](../guide/model.md) |
| 分页怎么做？ | 模型返回 `[$total, $list]`，视图用 `Helper::PageHtml($total)` | [第 2-5 章](../guide/database.md) |
| 事务怎么写？ | [`Helper::Db()->PDO()`](../reference/Db-Db.md) 上 `beginTransaction/commit/rollBack`；注意同一连接 | [第 2-5 章](../guide/database.md) |
| 缓存怎么生效？ | 默认 [`Cache`](../reference/Component-Cache.md) 是空实现；装 [`RedisCache`](../reference/Component-RedisCache.md) 才有缓存（`Helper::Cache()` 用法不变） | [第 2-13 章](../guide/cache.md) |

### 横切：钩子、事件、中间件

| 问题 | 答案 | 去哪看 |
|---|---|---|
| 想在请求前后做事，用哪个？ | 拦截用路由钩子（pre 链返回真值）；前后置装饰用中间件 | [第 2-10 章](../guide/lifecycle.md) |
| 中间件里 `return` 了响应怎么没生效？ | 已知坑：短路无效，控制器照样执行；要拦截请用钩子 | [第 2-10 章](../guide/lifecycle.md) |
| 想在所有控制器前做点什么，要继承基类吗？ | 先考虑钩子；继承会把可变能力变成血缘 | [第 2-10 章](../guide/lifecycle.md) |
| 事件和钩子区别？ | 事件是广播（无返回值、可跨相位）；钩子是单链（有返回值、可短路） | [第 2-12 章](../guide/events.md) |
| 怎么知道谁挂了钩子、顺序如何？ | [`RouteHookManager::_()->dump()`](../reference/Ext-RouteHookManager.md) | [第 2-10 章](../guide/lifecycle.md) |
...
| 生产环境怎么部署？ | 文档根指向 `public/`，rewrite 到入口；关 `is_debug`；`runtime/` 可写 | [第 1-7 章](../guide/deployment.md)、[第 2-17 章](../guide/security-performance.md) |
| 内置 HTTP 服务器能上生产吗？ | 不能，只适合开发/内网 | [第 4-5 章](../guide/http-server.md) |
| 怎么加定时任务？ | 写 `command_xxx()` 命令，crontab 里 `cd 项目目录 && php cli.php xxx` | [第 2-15 章](../guide/cli.md) |

### 测试与文档

| 问题 | 答案 | 去哪看 |
|---|---|---|
| 测试怎么跑？ | 测试一律在 WSL/容器里跑；日常只跑单个文件 | [第 2-16 章](../guide/testing.md) |
| 为什么 Windows 下测试失败？ | Windows PHP 没有 redis 扩展等环境差异 | [第 2-16 章](../guide/testing.md) |
...
| 症状 | 先去哪 | 最可能的原因 |
|---|---|---|
| 404 | [第 2-2 章](../guide/routing.md) §7 | 前缀不符（E001）/ 类不存在（E003）/ 单段路径规则 |
...
| 视图里变量是 `null` | [第 2-4 章](../guide/views.md) | 没传进 `Show()`；`view_skip_notice_error` 掩盖了 notice |
...
| 生产看到堆栈 | [第 2-17 章](../guide/security-performance.md) | `is_debug`（含子应用）或 `duckphp_is_debug` |
...
| 登录后还是未登录 | [第 2-18 章](../guide/user.md) | `globaluser_login_session` 键名/相位不对（旧名 `user_callback_for_*` 已失效） |
...
| 子目录部署后链接 404 | [第 2-2 章](../guide/routing.md) | 手写绝对路径，没用 `__url()` |
| 测试在 Windows 上假失败 | [第 2-16 章](../guide/testing.md) | 环境差异（redis 等），换 WSL |
| `ZAllDemoTest` 的 `files` 变红 | [第 2-16 章](../guide/testing.md) | 输出字节长度随源码变化，更新期望值 |
