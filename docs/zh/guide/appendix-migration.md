...
| 路由 | `UrlManager` 规则 | `Config\Routes` | `routes/web.php` | 约定（路径 → `Controller\Xxx::method`）+ `route_map` / 钩子（[第 2-3 章](../guide/routing.md)） |
...
| 中间件 | `behaviors()` / filters | Filters | Middleware | 钩子链为主；中间件只是兼容扩展（[第 2-4 章 路由钩子](../guide/route-hooks.md)） |
