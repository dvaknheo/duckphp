...
- **预置值是相位级的共享状态**：写进去之后，**同一相位后续每一次渲染**都会带上它（包括页眉页脚视图，以及同相位子应用的视图）。适合 `site_name`、当前用户这类「整站一样」的数据；请求独有的数据请走 `Show()` 的 `$data` 参数。
- **别把敏感数据放进去**：它会被所有视图看见；`__h()` 只解决转义，不解决「这个视图该不该看到」。
- **想清干净**：`View::_()->reset()`（清 `$data` 与页眉页脚设置）；或干脆只用每个动作自己的 `$data`，不依赖预置。
