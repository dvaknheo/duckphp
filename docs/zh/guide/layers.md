...
| 层 | 一句话职责 | 别做 |
|---|---|---|
| System | 接线 + 共享内核（**不属于四层**） | 不跳过 `*Action` 直接调 Business/Model；四层也别反向调它的接线动作 |
...
一句话记住越界矩阵的结论：**控制器不碰 Db/Model、业务不碰请求上下文、视图只读不写**；`Db`/`Session`/`Service`/`View` 都不占矩阵的行列，规矩在 1-3 章用几句话交代；越界的代价不是报错，而是覆盖、多应用、CLI/测试复用这些能力悄悄失效。
