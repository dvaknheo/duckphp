...
| **不跟 PSR 标准（无 PSR-7/15）**                                         | 保持零依赖、可单文件嵌入；请求/响应就是数组与简单对象                                 | 中间件只是**兼容层**（[`Ext\MyMiddlewareManager`](../reference/Ext-MyMiddlewareManager.md) 的 request 默认是 `stdClass`），别拿 Laravel / ThinkPHP / PSR 的心智模型硬套（[第 2-4 章](route-hooks.md)） |
...
| 坑                                                                                      | 现象                                                                                                                      | 现状 / 怎么绕                                                                                                                                                |
| -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **中间件短路靠返回值判定**                                                                            | 中间件不调 `$next` 时：返回响应（字符串）或 `true` ⇒ 已处理（控制器不再执行，管理器把字符串 `echo` 出去）；返回 `null`/`false` ⇒ 视为「没处理」，照旧放行给默认路由 | 刻意如此：给「忘了调 `$next`」留缓冲，不会突然白屏。**这一条曾经是坑（短路无效、控制器照跑、响应被丢弃），2026-09-26 已修**（[第 2-4 章](route-hooks.md) §5、[第 4-13 章](ext-classes.md) §4） |
...
| 两个 docker 测试目录里都没有 `.sh` 脚本                                                        | 那套起停脚本（只在 `docker/test-php84/` 里存在过）已删；两个目录都是纯 Dockerfile + compose                                                   | 用 `docker-compose run --rm fulltest` 起（[第 4-7 章](coverage.md) §4）                                                                                        |
...
- [第 4-13 章 `Ext\*` 扩展类](ext-classes.md)：`src/Ext/` 下的可选组件，含带 `@todo deprecate` 的过时类与"能用但不在推荐路径上"的兼容层。
