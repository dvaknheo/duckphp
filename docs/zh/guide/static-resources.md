...
> 前置：[第 27 章 把外部应用挂进来](mount-app.md)。预计 12 分钟。
...
GET /shop/res/third.css      → 父应用 res/shop/third.css（覆盖了子应用的同名资源，见第 30 章）
```
...
res/                    ← 主应用资源源目录
├── main.css
└── shop/               ← ★ 与被挂应用 name 同名的目录 = 覆盖它的资源（第 30 章）
    └── third.css
...
Helper::Res('main.css');           // 等价写法（第 15 章）
__url('shop/');                    // 生成普通 URL
```

## 与内置 HTTP 服务一起用

`HttpServer`（第 37 章）的文档根是 `path_document`：
...
| 现象 | 原因 | 改法 |
|---|---|---|
| 资源 404，文件明明存在 | 前缀斜杠不对（多一个 / 或少一个 /） | 根应用 `'/res/'`、子应用 `'res/'`；用第 30 章的方法确认命中了哪个文件 |
...
| 两个应用资源同名被互相覆盖 | 共用一个前缀目录 | 按应用分前缀；覆盖是有意为之时才用同名（第 30 章） |

## 相关参考

- [DuckPhp\Component\RouteHookResource](../reference/Component-RouteHookResource.md)（含 `cloneResource()` 细节）
- [第 17 章 会话与用户/管理员体系](external-auth.md) 的 `use_admin_view` / `use_user_view`
- [第 30 章 重写与覆盖](overriding.md)：`res/<name>/` 覆盖规则
