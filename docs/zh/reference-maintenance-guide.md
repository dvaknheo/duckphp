...
- **现状**
  ...
  - 测试基线（WSL）：全量 `php vendor/bin/phpunit` → 上次记录 `OK (96 tests, 823 assertions)`；`XDEBUG_MODE=coverage` + `tests/support.php` → `test_reports/index.html` `Lines 4895/4895 (100.00%)`。**这两个数是上一次全量跑的记录**，重跑后请更新。`tests/data_for_tests/ZAllDemoTest.config.php` 里 `files` 的期望长度是 **10432**（跟当前工作区的选项表绑定，见第 7 节那一行）。
