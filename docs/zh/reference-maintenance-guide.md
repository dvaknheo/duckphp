...
| `docs/scripts/gen-architecture-gv.php` | **从 `src/` 生成架构图源文件 `docs/duckphp.gv`**（原来是手写的，2024-04 之后就烂了：里面还有 `DuckPhp\Helper`/`HelperX`/`HelperY`/`FastInstaller`/`Foundation\Core` 这些不存在的命名空间）。用 nikic/php-parser 扫描：一个命名空间一个 cluster（标签带 type 数）、形状按声明种类（class=box / abstract=box3d / interface=note / trait=diamond）、边按关系（extends 实线 / implements 虚线 / `use <Trait>` 点线 / `initComponents*()` 里出现的 `::class` 画成蓝色 `assembles` 粗线），节点 tooltip 是源文件路径，输出确定性排序；`--check` 只校验是否最新（过期退出码 1），`--stdout` 打印。**整张图只输出英文**（图例、簇标签、边标签、脚本注释都是英文，零非 ASCII ⇒ 不挑字体）。**渲染**：`dot docs/duckphp.gv -T svg -O` → `docs/duckphp.gv.svg`（LR + `newrank=true`，实测 1608×3624pt）。⚠️ 2026-09-26 这台机器 **WSL 与 Windows 都没装 graphviz**，当时用 npm 上的 WASM 版渲染（`npm i @viz-js/viz`；Node 里 `require('@viz-js/viz').instance().then(viz => viz.renderString(src, {format:'svg'}))`，graphviz 16.0.0；该构建**不支持 png**，只出 svg/text 类格式）——换机器重渲染前先 `which dot`，没有就照这个方式。 |
...
- **现状**
  ...
  - 架构图：`docs/duckphp.gv`（由 `gen-architecture-gv.php` 生成）+ 渲染出的 `docs/duckphp.gv.svg`，现为 **13 个命名空间簇 / 107 个类型 / 97 条边**，跟 `src/` 一一对应（删掉 `Ext\MiniRoute`/`Ext\Misc` 后图里就没有它们了）；**图内文字全英文**（图例 / 簇标签 / 边标签），零非 ASCII。`--check` 已进第 9 节的冒烟自检。注意这份图**目前没有任何中文文档链到它**（`docs/old/`、`backup/` 里的旧链接不算），只在维护指南里维护。
