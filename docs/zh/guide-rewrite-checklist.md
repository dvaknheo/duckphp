...
> **章号形式**：全库统一用**卷-章号**——第一卷 `1-1`–`1-7`、第二卷 `2-1`–`2-17`、第三卷 `3-1`–`3-7`、第四卷 `4-1`–`4-10`，附录 A/B/C/D。**单数字章号已废除**（历史上第二卷曾用 8–24 的单数字，与最初 9/14/15/16/17 的排法做过一次置换）。
> 「中间件与钩子链」**不再单独成章**（中间件只是兼容性扩展，不是主推能力）——已并入 2-10 作为一节，其余章顺次前移；全书共 41 章 + 4 附录。
...
- [x] 2-7 Helper 与全局函数 —— `helper.md`（177 行：四个 trait 分工表、全局函数全表、`assignRewrite` 前导 `/` 已修）
- [x] 2-8 表单与验证 —— `validator.md`（180 行：三种口径、`valid/check/filter`、skipEmpty、未知规则即抛）
- [x] 2-9 会话与用户/管理员体系 —— `external-auth.md`（228 行：SessionTrait、`user_callback_for_*` 新键名、PermissionMenu、视图开关）
