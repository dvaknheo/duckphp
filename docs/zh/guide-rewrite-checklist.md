...
- [x] 1-3 目录结构与四层架构 —— `project-structure.md`（M15 把原 2-1 的四层规范并进来：目录结构 + 命名规范 + 五层职责 + 越界矩阵 + 违规示例 + Helper 分层 + 全局函数表）
...
> **章号形式**：全库统一用**卷-章号**——第一卷 `1-1`–`1-7`、第二卷 `2-1`–`2-20`（2-2 请求生命周期 / 2-3 路由进阶 / 2-4 路由钩子 / 末尾 2-19 使用用户系统 / 2-20 使用管理员系统）、第三卷 `3-1`–`3-7`、第四卷 `4-1`–`4-13`（末尾 4-12/4-13 是实现用户/管理员系统），附录 A/B/C/D。**单数字章号已废除**（历史上第二卷曾用 8–24 的单数字，与最初 9/14/15/16/17 的排法做过一次置换）。
...
- [x] 2-1 四层架构与调用规范 —— `layers.md`（**M15 起是指路页**：正文已并入 1-3，本页留速查表 + 链接，目的是不必重排卷二章号）
- [x] 2-2 请求生命周期 —— `lifecycle.md`（M13 拆出：`init()` 八步 + **框架默认装了哪些内置组件**（root/inner/ext 三层表 + 只占位/只建实例的语义）+ `serve()` 时序 + `onBeforeOutput()`）
- [x] 2-3 路由进阶 —— `routing.md`（默认规则表、PATH_INFO 来源、`__url()` 规则、重写与路由映射、错误码速查）
- [x] 2-4 路由钩子 —— `route-hooks.md`（M13 新写：六个位置与短路语义、`RouteHookManager` 增删改查、内置钩子位置表、选型表。排号时与 2-3 路由进阶对调过：钩子的前置是路由）
...
- [x] 4-11 过时与冷门的扩展类 —— `deprecated-exts.md`（122 行，M8 新增）
- [x] 4-12 实现用户系统 —— `impl-user.md`（214 行，M12 新增：三件实现 + 选项表 + `ext` 挂载 + 自检）
- [x] 4-13 实现管理员系统 —— `impl-admin.md`（173 行，M12 新增：与 4-12 的差异清单 + `isSuper()`/`onNeedPermission()` + 菜单契约）
