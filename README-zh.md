...
**DuckPhp 以库方式引入**，所以项目骨架不像其他框架那样包含一大堆不可删除的文件。

**DuckPhp 零依赖。** 你不需要担心第三方包的变更。它无需引入 101 个包就能工作，稳定性完全可控。

`./vendor/bin/duckphp new` 命令会复制骨架文件，并根据你在 `composer.json` 中设置的 `src` 命名空间替换项目命名空间。
...
DuckPhp 通过 Composer 包 `dvaknheo/workermanhttpd` 扩展支持 Workerman，不需要修改项目代码即可运行，未来还会支持更多平台。

**DuckPhp 不限制你的项目的命名空间**
...
这是 DuckPhp 的一个重要特点。你无需对现有的 DuckPhp 应用进行二次开发。你可以直接将其作为插件使用。
...
这只是一个简单的嵌入演示。涉及 静态资源、 组件共享应用间通信、 重写内容、等复杂问题，详情请参阅文档。
