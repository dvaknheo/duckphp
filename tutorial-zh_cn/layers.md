...
Controller 层 (可处理请求上下文)
  ...
  ├── Helper              静态辅助方法
  ├── Session             纯状态容器（不调其他类）
  └── Action             通用功能（调 Business + Session，❌ 不调 Model）
                                │
Business 层 (纯无状态)           ▼
...
| 从 \ 到 | Session | Action | Controller | Business |  Model |
|---|---|---|---|---|---|
| **Controller** | ✅ | ✅ | ❌| ✅ |  ❌ |
| **Action** | ✅ | ✅ | ❌ | ✅ |  **❌** |
| **Business** | **❌** | — | — | ✅ | ✅ | ✅ |
...
- **Controller/Action** 可以读写 Session，**不能直接调 Model**

- **Service** 是 Business 层的通用功能，可被多个 Business 调用
- **Action** 是 Controller 层的通用功能，可被多个 Controller 调用
...
- 决定输出（视图/JSON/重定向等）
- 调用集中管理的 Session 类
- Controller之间互不调用，如果需要部分功能复用，请放入Action
### 基础用法
...
Controller 层
  ...
  ├── Helper             静态辅助方法
  ├── Session            纯状态容器
  └── Action ★           可复用的通用功能 → 调 Business + Session
                              │
Business 层                 ▼
  ├── UserBusiness        业务逻辑 → 调 Model
  ├── Service ★           可复用的通用功能 → 调 Model + Business
  └── Helper              静态辅助方法
                              │
Model 层                    ▼
  └── UserModel           数据访问
...
- ✅ 同一套编排被 **多个路由方法** 或 **多个 Controller** 共用
- ✅ 涉及 **Session + Business** 的组合操作
- ❌ 仅调用一次 Business 的简单透传 → 直接在 Controller 中调 Business 即可
- ❌ 纯 Session 读写 → 直接调 Session 即可
...
Business 层
  ├── UserBusiness     用户业务逻辑
  ├── OrderBusiness    订单业务逻辑
  ├── CommonService ★  共享服务 → 可被多个 Business 调用
  └── Helper           静态辅助方法
```
